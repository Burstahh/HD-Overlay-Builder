...
powershell -NoProfile -ExecutionPolicy Bypass -Command "Compress-Archive -Path 'av_variants\singlefile_selfcontained\*' -DestinationPath 'av_variants\HD_Overlay_Builder_v1.4.2_singlefile_selfcontained.zip' -Force; Compress-Archive -Path 'av_variants\onefolder_selfcontained\*' -DestinationPath 'av_variants\HD_Overlay_Builder_v1.4.2_onefolder_selfcontained.zip' -Force; Compress-Archive -Path 'av_variants\onefolder_frameworkdependent\*' -DestinationPath 'av_variants\HD_Overlay_Builder_v1.4.2_onefolder_frameworkdependent.zip' -Force; Get-ChildItem 'av_variants' -Filter '*.zip' | ForEach-Object { (Get-FileHash $_.FullName -Algorithm SHA256).Hash + '  ' + $_.Name } | Set-Content 'av_variants\SHA256SUMS.txt'"
